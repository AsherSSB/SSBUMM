...
from custom.logger import PlayerReportLogger, BugReportLogger
...
class Controller(commands.Cog):
    ...
    async def match_players(self, player1:tuple[int, discord.Interaction], player2:tuple[int, discord.Interaction]):
        ...
        try:
            ...
            if stages[1] == 1:
                ...
                room = await self.send_host_view(player1[1])
                result = await self.game_loop(player1[1], player2[1], p1confirm[1], p2confirm[1], room, stages[0])
            else:
                await player1[1].followup.send("Your opponent is preparing the arena, please wait...", ephemeral=True)
                room = await self.send_host_view(player2[1])
                result = await self.game_loop(player2[1], player1[1], p2confirm[1], p1confirm[1], room, stages[0])
                if result == 1:
                    result = 0
                else:
                    result = 1
    ...
    async def game_loop(self, player1:discord.Interaction, player2:discord.Interaction, p1char, p2char, room, starter):
        ...
        while 2 not in score:
            try:
                p1emb = asyncio.create_task(self.send_embed(player1, player2, starter, p1char, p2char, score, room))
                p2emb = asyncio.create_task(self.send_embed(player2, player1, starter, p2char, p1char, score, room))
            ...
            if 2 not in score:
                stagelist = ("Battlefield", "Small Battlefield", "Final Destination", "Hollow Bastion", "Town and City", "Smashville", "Kalos Pokemon League", "Pokemon Stadium 2")
    ...
    async def send_embed(self, player:discord.Interaction, opponent:discord.Interaction, 
                         stage:str, p1char:str, p2char:str, score:list[int], room:str): # score[0] NEEDS to be player's score and score[1] opponent score
        try:
            ...
            embed.add_field(name=f"Room Code:", value = f"{room}", inline=True)
